      ...
      if (selectedGroupTopicId) {
        ...
        GroupHomeworks.forEach((GroupHomework) => {
          const isExisting = lsc.engine.entities.some(
            (e) =>
              e.get(IdentifierFacet)?.props.guid === GroupHomework.id &&
              e.hasTag(DataTypes.GROUP_HOMEWORK),
          );

          if (!isExisting) {
            const GroupHomeworkEntity = new Entity();
            lsc.engine.addEntity(GroupHomeworkEntity);
            GroupHomeworkEntity.add(
              new TitleFacet({ title: GroupHomework.title }),
            );
            GroupHomeworkEntity.add(
              new IdentifierFacet({ guid: GroupHomework.id }),
            );
            GroupHomeworkEntity.add(
              new DateAddedFacet({ dateAdded: GroupHomework.date_added }),
            );
            ...
            GroupHomeworkEntity.add(
              new DueDateFacet({ dueDate: GroupHomework.date_added }),
            );
