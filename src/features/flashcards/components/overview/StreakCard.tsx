...
const StyledStreakLabel = styled.div<{ currentStrak: boolean }>`
  ...
  ${({ currentStrak }) =>
    currentStrak ? tw`text-4xl` : tw`text-2xl mt-2 leading-7`}
`;
...
const StreakCard = () => {
  ...
  return (
    <StyledCardWrapper>
      <div>
        <StyledText>
          {isCurrentStreakEntityExisting ? "Aktuelle Streak" : ""}
        </StyledText>
        <StyledStreakLabel currentStrak={isCurrentStreakEntityExisting}>
          {isCurrentStreakEntityExisting
            ? `${streak} Tage`
            : "Bereit für eine neue Streak?"}
        </StyledStreakLabel>
      </div>
      <StyledCheckmarksContainer>
        {Array.from({ length: 7 }).map((_, index) => {
          const isChecked =
            index >= streakStartIndex && index <= streakEndIndex && streak > 1;
          return (
...
const useCurrentStreak = () => {
  ...
  const dateUpdated =
    currentStreakEntity?.get(DateUpdatedFacet)?.props.dateUpdated || 0;
