...
export async function addBlockEntitiesFromString(
  ...
  let splitRegex =
    /<div>|<div\s*\/?>|<br\s*\/?>|<\/div>|<li>|<\/li>|<p>|<p\s*\/?>|<\/p>/g;

  let contentBlocks = cleanedHtmlString
    .split(splitRegex)
  ...
  contentBlocks.forEach((content, index) => {
    const isExisting = lsc.engine.entities.some(
      (e) =>
        ...
        e.get(TextFacet)?.props.text === content.replace(/<[^>]+>/g, "").trim(),
    );
    ...
    if (!isExisting) {
      ...
      const isBold =
        /^(<b(\s|>).*<\/b(\s|>)|<strong(\s|>).*<\/strong(\s|>))/i.test(
          trimmedContent,
        );
